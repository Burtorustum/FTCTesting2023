...
public class PIDFController {
    ...
    public double update(double target, double error) {
        ...
        if (coeff.lowPassGain > 0 && coeff.lowPassGain < 1) {
            double currentFilterEstimate = (coeff.lowPassGain * previousFilterEstimate) + (1 - coeff.lowPassGain) * errorChange;
        ...
        return (coeff.kp * error) + (coeff.ki * integralSum) + (coeff.kd * derivative) + (coeff.kf * target);
    }
}
