...
public class ConeDetector extends Vision {
  ...
  public final OpenCvPipeline pipeline = new OpenCvPipeline() {
    ...
    public Mat processFrame(Mat input) {
      ...
      // Blur / Erode / Dilate
      Imgproc.blur(PipelineStage.CROP.result, PipelineStage.BLUR.result, blurKSize,
          new Point(-1, -1));
      Imgproc.erode(PipelineStage.BLUR.result, PipelineStage.ERODE.result, new Mat(),
          new Point(-1, -1), erodeIterations, Core.BORDER_DEFAULT);
      Imgproc.dilate(PipelineStage.ERODE.result, PipelineStage.DILATE.result, new Mat(),
          new Point(-1, -1), dilateIterations, Core.BORDER_DEFAULT);

      // Convert Color
      Imgproc.cvtColor(PipelineStage.DILATE.result, PipelineStage.CVT_COLOR.result,
